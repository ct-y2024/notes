...
1) Они разрабатываются разными людьми с разными целями. C++ используется во многих крупных проектах, таких как компиляторы, операционные системы, игры, а также в области разработки приложений и веб-разработки. C, с другой стороны, используется в большинстве системного программирования, встраиваемых системах и в области разработки операционных систем.
...
3) Хотя эти языки имеют общие базовые конструкции, стиль и приёмы программирования на них сильно отличаются.
...
`(a ? b : c) = 42`, а в C++ — `a ? b : (c = 42)`.
 ...
 На курсе всё будет обсуждаться в терминах C++: в местах отличий мы не будем обсуждать оба языка.
...
Это не будет так работать, как вы хотите, потому что, как показано выше, на разных ОС могут быть разные размеры целочисленных типов. Поэтому нужно использовать хедер  `cstdint` 
```c++
#inlcude <cstdint>
int8_t			uint8_t
int16_t			uint16_t
...
int64_t			uint64_t
```
Их следует использовать, если хотим тип гарантированного размера. Эти типы существуют в том и только в том случае, если реализация имеет тип соответствующего размера. В частности, в патологическом случае указанном выше (где байт имеет 64 бита), из типов фиксированного размера существуют только `int64_t` и `uint64_t`.

#### Прочие полезные `typedef`'ы.
Предположим вам нужно перебрать все значения в массиве:
```c++
for (/*???*/ i = 0; i < N; i++)
	arr[i] = 42;
```
