...
\end{pmatrix}= S \begin{pmatrix}
    ...
    {w}^n
\end{pmatrix}$$
Пусть $T_{e \to e'}=S^{-1}=T=(t^i_j)_{n \times n}$ - невырожден. $\Rightarrow e' = eT$ базис в пр-ве $V$.
...
\textbf{Следствие.} $e,e'$ базисы $V$. $w,w'$ сооотв. сопряженные базисы к $e,e'$ в $V^*$. 

$T = T_{e\rightarrow e'}, S = T^{-1} \Rightarrow \forall x \in V: \forall f \in V^*$
...
$(T_{w\rightarrow w'})^T = S$ - связь координат в старых и новых координатых
...
%там будет схемка со стрелками, это
...
$\forall f \in V^* \leftrightarrow (a_1,\ldots, a_n), a_j = f(e_j)$.
...
$\begin{pmatrix}
    ...
    \vdots \\
    w^n \\
\end{pmatrix} \cdot (e_1e_1 \dots e_n) = E $
% ?????
...
%вставить картинку и комментарий

Пусть $e_1,\dots e_n$ базис $V$

$\rightarrow "e_1",\ldots, "e_n"$. Покажем, что это базис в $V^{**}$.

Мы знаем, что $\dim (V^*)^* = \dim V^* = \dim V = n$. Откуда достаточно показать, что $"e_1",\ldots, "e_n"$ - линейно независимы.

$0= \sum\limits_{i=1}^n \alpha_i "e_i"(w_i)=\sum\limits_{i=1}^n\alpha_i w^j(e_j) = \alpha_j \Rightarrow$ линейно независимы, откуда базис.

$x \in V \leftrightarrow "x" \in V$ - изомофризм
...
$e_j(f)=f(e_j)=a_j(f \in V^*)$

$e_j \in V^{**}$ - коорд. формы относительно базиса $w^i$.
...
$\Rightarrow \forall x \in V: w^j(x) =x^i: x^iv_j = x$ %я нихуя не понимаю пупупупупупупууп. приехали. Я попробую попонимать, а потмо ели что перейду в режим теха конец
