...
$x\in \mathbb{R}(\mathbb{C})$ --- их область определения, $|x| < \mathbb{R}$ --- \deff{радиус сходимости}, $c_m \in \mathbb{R}(\mathbb{C})$. Причем эти $c$-шки на самом деле производные. (если интересно см. конспект по мат. анализу первый семестр)
...
\thmm{Теорема 2 (функция от диагонализируемой матрицы 2)}
...
Назовем $V_{\mathbb{C}}$ \deff{комплесификацией} V
...
\deff{def:} $z \in V_c$, $\overline{z}=x-iy$ - \deff{сопряженный} вектор, $z=x+iy, \quad x,y \in V$
...
\deff{def:} Возьму оператор $\mathcal{A} \in End(V)$.  $\forall z \in V_c: \mathcal{A}_cz =\mathcal{A}x + i \mathcal{A}y$ 
...
\thmm{Теорема: (о существовании минимального аннулятора)}
\begin{enumerate}
...
\deff{def:} Нормализованный многочлен $\psi(t)$ называется \deff{аннулятором оператора} $\mathcal{A}$, если:
