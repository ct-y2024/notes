...
Зафиксируем $\varphi$ и $\lambda$ и назовем  \deff{главным идеалом}, порожденным многочленомом $\varphi_\lambda$:
...
\thmm{Теорема (Разложение Жардана)}
...
Пусть $\mathcal{A} = \mathcal{D}'  + \mathcal{C}$, $D'C=CD'$



$\mathcal{A} = \sum\limits_{\lambda}\lambda P_{\lambda} + B$

Нужно доказать, что $\{\lambda\} = \{\mu\}$, а также $\Im Q_\mu = K_{\lambda} = \Im P_{\lambda} = \ker (\mathcal{A}-\lambda \varepsilon)^{m(\lambda)}$
...
$$(\chi_{\mathcal{A}}(\mu))^{\nu-1} = \chi_{\mathcal{D}}(\mu) (\chi_{\mathcal{A}}(\mu))^\nu)$$
...
Возьмем $K_r = \ker(B^r) = \ker((\mathcal{A}-\lambda\varepsilon)^r),r= 1,\ldots, m: K_1 \subseteq K_2 \subseteq\ldots \subseteq K_m = K  =K_1= K_{\lambda}$. Заметим, что $m$ - минимальная степень, когда он зануляется. Докажем, что там строгое включение:
...
Теперь мы приходим к \deff{матрице в форме Жордана}, она состоит блоков верхнего уровня, соответсвующих собственных чисел
$$J = \begin{pmatrix}
    J(\lambda) & & & \\
    & J(\mu) & & \\
    & & \ddots & \\
...
Заметим, что в таком случае: $z_0 \subseteq z_1 \subseteq \ldots \subseteq z_m $, а также $z_{r+1}= z_r \oplus \overline{K}_{r+1}$, где $\overline{K}_r$ - \deff{опорное подпространство} - отличие между $K_r$ и $K_{r-1}$ ($K_i \oplus \overline{K}_{i+1}=K_{i+1}$). Тогда заметим вот такую формулу:
...
$\forall r: 1\leq r \leq m-1$ будет выполнено: $B^rK = B^{r+1}K \oplus B^r K_{r+1}\oplus B^r K_{r+2}\oplus\ldots\oplus B^r K_m$
...
$$B^rK = B^{r+1}K + B^r K_{r+1}+ B^r K_{r+2}+\ldots+ B^r K_m$$
...
Но, как мы помним $BK$ и $\overline{K}_j$ - дизъюнктны. То есть $x_1=x_2 =\ldots = x_m =0$. Откуда получаем, что $Bx$ тоже ноль, откуда разложение нуля - тривиально.
...
$ BK = B^2K \oplus B\overline{K}_1\ldots \oplus B\overline{K}_m$\\
$\vdots$\\
...
$\overline{K}_r \oplus B\overline{K}_r \oplus \ldots \oplus B^{r-1}\overline{K}_{r} = \tau_r$ - называется \deff{башней} 
...
$\dim \overline{K}_r = \dim B \overline{K}_r = \ldots = B^{r-1}\overline{K}_r = d_r$.
...
Пусть $x \in \ker B^j$ и $x\in \overline{K}^r$, тогда $x\in \ker B^j \cap \overline{K}_r = K_j \cap \overline{K}_r$. А как мы знаем $ K_j \cap \overline{K}_r = \{\zero\}$(достаточно посмотреть на рисунок, $K_j$ соответствует $j$-ому и меньшим этажам). А это значит, что $x \in \{\zero\} \Leftrightarrow x =\zero$, откуда ядро тривиально, наша функция инъективна, а из этого уже следует изоморфность, то есть биекция.
...
\deff{def:} \deff{Циклическим базисом}, порожденным вектором длины $r$ называются $g_p,Bg_p,\ldots,B^{r-1}g_p$. В таком случае $Bg_p,\ldots,B^{r-1}g_p$ называют \deff{присоединяемыми}.
...
$$A\Big|_S  \xleftrightarrow{j}\begin{pmatrix}
    \lambda & 1 & &\\
     ...
     0& &  &\lambda 
\end{pmatrix}$$
$V = \bigoplus\limits_{\lambda\text{ - с.ч.}} K_\lambda = \bigoplus\limits_{\lambda}\bigoplus\limits_{r}\bigoplus\limits_{p}S_{p,\lambda,r}$ - объединение всех базисов называется \deff{Жордановым базисом}.

\sout{стоит ли так вставлять рисунок?}